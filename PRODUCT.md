...
Single page, single action. The visitor lands, an episode is already chosen, and one button draws another. Every draw is a full server round trip today. There is no history, no favourites, no queue; a refresh is a new episode. Typical session is a handful of draws at most.
...
- Scope is fixed at one click: no filters, no show picker, no season narrowing. The design carries the experience.
...
1. One episode, never a list. The moment the surface offers a comparison it has failed at its only job.
